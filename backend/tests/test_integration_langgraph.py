...
def mock_llm_registry():
    ...
    with patch("app.ai_interviewer.llm_providers.get_llm_registry") as mock:
        ...
        responses = {
            ...
            "opening": {
                "opening_text": "Hi Test, I'm Jack, your interviewer. I've reviewed your background and today we'll discuss your Python experience in a little more depth.",
            },
            "question_generator": {
...
class TestInterviewGraphFlow:
    ...
    @pytest.mark.asyncio
    async def test_new_question_is_generated_after_answer(self, mock_llm_registry, initial_state):
        """A question should advance after the candidate answers, not reuse the same active record."""
        runner = InterviewGraphRunner(
            session_id=initial_state["session_id"],
            initial_state=initial_state,
        )
        ...
        previous_question_id = runner.state["current_question"]["id"]

        result = await runner.process_answer(
            "I handled migrations with Alembic and rollback checks to protect production data.",
            code_snapshot=None,
        )

        assert runner.state["current_question"]["id"] != previous_question_id
        assert runner.state["current_answer"]["question_id"] == previous_question_id
...
class TestVoicePipelineIntegration:
    ...
    async def test_stt_fallback_chain(self, initial_state):
        ...
        from app.ai_interviewer.voice import VoicePipeline, WhisperSTT
    ...
    async def test_tts_fallback_chain(self, initial_state):
        ...
        from app.ai_interviewer.voice import OpenAITTS, VoicePipeline
