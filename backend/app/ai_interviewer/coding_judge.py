...
async def _run_judge0(
    ...
    """Run code against test cases via Judge0 API or unified execution engine."""
    import httpx
    ...
    # Try direct Judge0 API if configured
    if language_id and (settings.judge0_host or settings.judge0_url):
        ...
        try:
            async with httpx.AsyncClient(timeout=timeout + 5.0) as client:
                for case in test_cases:
                    ...
                    try:
                        ...
                        break  # Fall back to unified execute_code below
                    ...
                    if status_id == 3:  # Accepted
                        ...
                        break
                    elif status_id == 5:  # Timeout
                        status = "timeout"
                        output = stderr or "Execution timed out."
                        error = ""
                    elif status_id in (7, 10, 11):  # Runtime error
                    ...
                    if status != "passed" and normalize_output(stdout) == normalize_output(expected):
                        status = "passed"
                        passed += 1
                        output = stdout
                        error = ""
                ...
                if len(results) == len(test_cases) or compile_error:
            ...
            pass  # Fall back to execute_code loop below

    # Unified code_executor fallback (Judge0 -> local runner)
    results = []
    passed = 0
    compile_error = ""
    ...
    for case in test_cases:
        ...
        if not run.get("ok"):
            ...
            output = stderr or "Execution timed out."
            error = ""
        elif status_id == 6 or "SyntaxError" in stderr:
